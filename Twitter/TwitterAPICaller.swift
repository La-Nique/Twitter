...
class TwitterAPICaller: BDBOAuth1SessionManager {    // these classes manage the successs and failures of API requests
    ...
    func postTweet(tweetString:String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(myUrl, parameters: ["status": tweetString], progress: nil, success: { (task: URLSessionTask, response: Any?) in success()},
        failure: { (task: URLSessionTask?, error: Error) in failure (error)})
        }
    ...
    func favoriteTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(myUrl, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionTask, response: Any?) in success()},
                                      failure: { (task: URLSessionTask?, error: Error) in failure (error)})
        }
